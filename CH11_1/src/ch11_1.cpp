...
public:
    ...
    ostream& operator<<(ostream& stream, const SmartPhone& p);
};
//ostream& stream, const SmartPhone& p
ostream& SmartPhone::operator<<(ostream& stream, const SmartPhone& p){
	print(stream);
	return stream;
}
...
CppSTL::CppSTL(vector< Person* >& persons, int& cpCount):BasePM(persons, cpCount){
...
void CppSTL::top10WordCount() {
    ...
    for(auto& p : persons){
        ...
        for (size_t pos = 0; !memo.getNext(pos, next).empty(); )
        	wordCountMap[next] = wordCountMap[next] + 1;
    }
...
void CppSTL::run() {
    ...
    func_t func_arr[] = {
		...
		&CppSTL::reverseSort, &CppSTL::reverse, &CppSTL::CppSTL::dispMemo,
		&CppSTL::countWord, &CppSTL::top10WordCount, &CppSTL::copyPersons
    };
...
PersonManager::PersonManager(Person* array[], int len):BasePM(persons, cpCount), array{array}, arrLen{len}, cpCount{} {
